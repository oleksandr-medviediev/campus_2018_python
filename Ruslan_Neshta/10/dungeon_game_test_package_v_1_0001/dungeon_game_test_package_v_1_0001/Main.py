...
def play_game(game_save):
    ...
    if game_save != {}:
        ...
        game_enemy.load(game_save)

    separate_worker = threading.Thread(target=game_enemy.update, args=(game_player,))
    
    separate_worker.setDaemon(True)
    separate_worker.start()
