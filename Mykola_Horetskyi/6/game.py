...
def init_game():
    ...
    start_x = randint(0, map_size - 1)
    start_y = randint(0, map_size - 1)
...
def process_player_commands(dmap, discovered_map, pos_x, pos_y,
    ...
    if user_input == 'view map' or  user_input == actions['view map']:
        ...
        printed_map = deepcopy(discovered_map)
...
def save_game(dmap, discovered_map, pos_x, pos_y, treasure_collected):
    ...
    with open(SAVE_FILE_NAME, 'wb') as dungeon_save_file:
        dump(current_data, dungeon_save_file)
...
def load_game():
    ...
    with open(SAVE_FILE_NAME, 'rb') as dungeon_save_file:
        game_data = load(dungeon_save_file)
    ...
    while True:
        ...
        if isfile("./{}".format(SAVE_FILE_NAME)):
            ...
            if (user_input == "load"):
                ...
                logger.info(choice(text.load_game_start))
        ...
        if not is_game_started:
            ...
            logger.info(choice(text.go_to_dungeon))
            input()
            logger.info(choice(text.enter_dungeon))
            input()
